...
function loadProfileData() {
  ...
  if (profilePreview) {
    profilePreview.src = googleUser.picture || "/placeholder.svg?height=100&width=100"
  }
  ...
  if (displayNameInput) displayNameInput.value = googleUser.displayName || googleUser.name || ""
...
function handlePhotoUpload(event) {
  ...
  reader.onload = (e) => {
    ...
    googleUser.newProfilePicture = e.target.result
...
function handleProfileSave(event) {
  ...
  try {
    // Update user data
    googleUser.displayName = formData.get("displayName") || googleUser.name
    googleUser.bio = formData.get("bio") || ""
    googleUser.favoriteInstrument = formData.get("favoriteInstrument") || ""

    // Handle profile picture
    if (googleUser.photoRemoved) {
      googleUser.picture =
        "data:image/svg+xml;base64,"
    } else if (googleUser.newProfilePicture) {
      googleUser.picture = googleUser.newProfilePicture
    }
...
function updateUserInfoDisplay() {
  ...
  if (userName) {
    const tempBadge = googleUser.temporary
      ? '<span style="background: #ff6b6b; color: white; font-size: 0.7rem; padding: 0.2rem 0.5rem; border-radius: 10px; margin-left: 0.5rem;">TEMP</span>'
      : ""
    userName.innerHTML = `Hola, ${googleUser.displayName || googleUser.given_name || googleUser.name}${tempBadge}`
  }
}
